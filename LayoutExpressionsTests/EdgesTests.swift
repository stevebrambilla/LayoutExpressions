...
class EdgesTests: XCTestCase {
	...
	func testEdges() {
		let constraints = evaluateExpression(subview.lexEdges == container.lexEdges)
	...
	func testVariableInsets() {
		...
		let constraints = evaluateExpression(subview.lexEdges == container.lexEdges - insets)
	...
	func testEqualInsets() {
		let constraints = evaluateExpression(subview.lexEdges == container.lexEdges - 10.0)
	...
	func testInitializingInsetsFromUIEdgeInsets() {
		...
		let constraints = evaluateExpression(subview.lexEdges == container.lexEdges - EdgeInsets(insets))
